...
while flags_variables["run"]:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if flags_variables["pantalla_principal"]:
                ...
                if pregunta_elegida != None:
                    ...
                    contador_nivel = manejar_respuesta_seleccionada(flags_variables, contador_nivel, dict_pregunta_cargada,  dict_elementos_pantalla_juego)
                    corroborar = corroborar_checkpoint_y_victoria(contador_nivel,flags_variables, dict_niveles_premios, dict_general_pantallas_secundarias)
                    print(corroborar)
                    if corroborar:
                    ...
                    if not contador_nivel:
                        ...
                        dict_general_pantallas_secundarias = habilitar_game_over(flags_variables, dict_general_pantallas_secundarias, msj_error)
            ...
            if flags_variables["pantalla_guardar_score"] and flags_variables["botones_pantalla_guardar_score"]:
                if manejar_evento_guardar_score(event, contador_nivel, dict_niveles_premios, dict_general_pantallas_secundarias,flags_variables):
                    contador_nivel = resetear_juego(flags_variables,dict_elementos_pantalla_juego,contador_nivel)

                
                

            #manejar_evento_mostrar_score(event, flags_variables, diccionario_paths, dict_elementos_pantalla_principal)
    ...
    if flags_variables["pantalla_principal"]:
        ...
        if colision_lado_derecho(rectangulo_dibujado, rectangulo_text):
            flags_variables["colicion_rects"] = True
